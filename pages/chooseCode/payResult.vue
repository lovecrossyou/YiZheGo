		...
		<view class="product-info">
			...
			<view class="product-text">
				...
				<view class="product-price">
					<view v-if="orderDetails.price" class="product-old-price">¥{{orderDetails.price/100}}</view>
					<view v-else class="product-now-price">¥{{orderDetails.oneDiscountPrice/100}}</view>
				</view>
			</view>
		</view>
		...
		methods: {
			...
			async clientOrderDetail() {
				...
				if (this.productType === 'vipProduct') {
					orderDetail = await api.vipProductOrderDetail({
						vipProductPayOrderNo: this.payOrderNo
					})
				} else {
					orderDetail = await api.clientOrderDetail({
						payOrderNo: this.payOrderNo
					})
				}
