<template>
	<view class="recommend_wrapper">
		<view class="recommend_item" v-for="(item, i) in attentionList" :key="i" @click="goDetail(item.discountGameId)">
			<view class="recommend_item_img"><image :src="item.productImageUrl" alt="" mode="aspectFill" /></view>
			<view class="recommend_item_intro">
				...
				<view class="recommend_item_intro_sale">
					<view class="price">
						￥
						<span style="font-size: 36upx;">{{ item.oneDiscountPrice / 100 }}</span>
					</view>
					<view class="already_sale">
						<image :src="home_huo_icon"></image>
						已抢{{ item.participatePersonCount }}件
					</view>
				</view>
			</view>
		</view>
