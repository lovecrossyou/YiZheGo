<template>
	<view class="user_wrapper">
			...
			<view class="order_status_wrapper">
				<view class="order_status">
					<view class="wait_announce">
						...
						<view class="red_circle" v-if="getWaitPayClientOrder.totalCount != 0">{{ getWaitPayClientOrder.totalCount }}</view>
					</view>
					<view class="wait_announce">
					...
					<view class="wait_announce">
						...
						<view class="red_circle" v-if="getWaitCommentClientOrder.totalCount != 0">{{ getWaitCommentClientOrder.totalCount }}</view>
					</view>
				</view>
				<view class="right_item"><orderStatus img="../../static/me/me_order_btn_refund@2x.png" statusText="中签/退款" pageNo="4"></orderStatus></view>
			</view>
		</view>
		<view class="other_unctions"><itemList page="/pages/me/wallet/wallet" img="../../static/me/me_icon_wallet@2x.png" title="钱包"></itemList></view>
		<view class="other_unctions">
			...
			<view @click="goMoments"><itemList img="../../static/me/me_icon_dynamic@2x.png" title="我的动态"></itemList></view>
		</view>
		<view class="other_unctions">
		...
		<view class="contact"><button type="primary" open-type="contact">联系客服</button></view>
	</view>
</template>
...
export default {
	data() {
		return {
			...
			getWaitPayClientOrder: {
				totalCount: 0
			},
			getWaitCommentClientOrder: {
				totalCount: 0
			},
			redCircle: true
		};
	...
	methods: {
		...
		async getMyOrder() {
			let res = await api.getMyOrder({
				clientOrderType: 'waitOpenResultClientOrder' //待揭晓
			});
			this.getMyOrderNum = res;
		},
		async waitPayClientOrder() {
			let res = await api.getMyOrder({
				clientOrderType: 'waitPayClientOrder' //待付款
			});
			this.getWaitPayClientOrder = res;
		},
		async waitCommentClientOrder() {
			let res = await api.getMyOrder({
				clientOrderType: 'waitCommentClientOrder' //已揭晓
			});
			this.getWaitCommentClientOrder = res;
		}
	},
	onShow() {
		this.personalInfo();
		this.vipUser();
		this.getMyOrder();
		this.waitPayClientOrder();
		this.waitCommentClientOrder();
	}
};
</script>
